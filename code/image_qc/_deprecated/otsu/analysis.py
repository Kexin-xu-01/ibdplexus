"""
Diagnostic analysis for Otsu tissue-fraction filter.

Samples patches, computes the fraction of foreground (tissue) pixels after
Otsu thresholding, and generates a threshold-strip plot to help choose a
minimum tissue-fraction cutoff.

H&E background is bright/white → after Otsu, foreground = dark pixels.
Patches with low tissue fraction are mostly background or artefact.

Output: tissue_threshold_15_remove_artifact/otsu/analysis_otsu.png
...
OUT_DIR     = Path("/home/jovyan/kgbk271-ibd-volume/data/processed"
                   "/tissue_threshold_15_remove_artifact/otsu")
...
for h5_path in selected:
    ...
    try:
        ...
        for idx in chosen:
            ...
            scores.append(otsu_tissue_fraction(gray))
...
print(f"Tissue fraction: min={scores.min():.3f}  p5={np.percentile(scores,5):.3f}  "
      f"median={np.median(scores):.3f}  p95={np.percentile(scores,95):.3f}")
print("\nRemoval at each threshold:")
for t in THRESHOLDS:
    print(f"  tissue_frac < {t:.2f}: {(scores < t).mean()*100:.1f}% removed")
...
ax_h.hist(s_sort, bins=80, color="#4fc3f7", edgecolor="none", alpha=0.85)
ax_h.set_xlabel("Otsu tissue fraction", color="white", fontsize=9)
ax_h.set_ylabel("Count", color="white", fontsize=9)
ax_h.set_title(f"Tissue content distribution  ({len(scores)} patches from {N_SLIDES} slides)",
...
for row_i, (thresh, color) in enumerate(zip(THRESHOLDS, palette)):
    ...
    for col_i in range(N_EXAMPLES):
        ...
        if col_i < len(b_idx):
            ...
            ax.set_title(f"{s_sort[b_idx[col_i]]:.3f}", fontsize=6.5, color="#ef9a9a", pad=1)
        else:
    ...
    for col_i in range(N_EXAMPLES):
        ...
        if col_i < len(a_idx):
            ...
            ax.set_title(f"{s_sort[a_idx[col_i]]:.3f}", fontsize=6.5, color="#a5d6a7", pad=1)
        else:
            ax.set_facecolor("#0d1b2a")
