"""
Plot 3-arm AUC comparison for the at-20-cm cohort (828 matched patients).

Arms: imaging only (prism2_base), RNA-seq only (VST), RNA + imaging (concat raw).
Reads fold metrics from at20cm_fold_metrics.csv; saves a Nature-style horizontal
bar chart with ±1 SD error bars.

Usage
-----
    python at20cm_3arm_comparison.py

Output
------
    <REPORTS_DIR>/at20cm_3arm_comparison.png
"""
...
METRICS_CSV = (
    ...
    '08_09_at20cm_site_controlled/results/at20cm_fold_metrics.csv'
)
REPORTS_DIR = (
    '/home/jovyan/kgbk271-ibd-volume/training/cd_vs_uc/'
...
ARMS = [
    ('img_base_20cm',   'Imaging only\n(prism2_base, 2,560-d)', BLUE),
    ('rna_20cm',        'RNA-seq only\n(VST, 17,963-d)',         AQUA),
...
def plot(metrics_csv: str = METRICS_CSV, out_dir: str = REPORTS_DIR) -> str:
    ...
    ax.set_yticklabels([a[1] for a in ARMS], fontsize=6.5, color=INK,
                       fontfamily='sans-serif')
    ax.set_xlabel('AUC (5-fold CV, patient-level)', fontsize=7, color=INK,
                  fontfamily='sans-serif')
    ...
    ax.set_title('At-20-cm · 828 matched patients\n(H&E + RNA-seq)',
                 fontsize=7.5, fontweight='bold', color=INK, pad=6,
                 fontfamily='sans-serif', loc='left')

    plt.tight_layout(pad=0.4)
    out = os.path.join(out_dir, 'at20cm_3arm_comparison.png')
