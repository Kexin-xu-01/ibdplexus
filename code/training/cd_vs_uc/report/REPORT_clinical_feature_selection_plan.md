...
Train a clinical-variable-only classifier on the same proximity-matched cohort used
in `08b_train_at20cm_visit_level.py` (~817 patients), using only variables that a
gastroenterologist would have access to **at the time of diagnosis** — before any
disease-specific classification, treatment, or surgery is assigned.

**Cohort definition**: patients in `cv_splits_patients.csv` who have at least one
at-20-cm H&E slide and at least one at-20-cm RNA-seq sample collected within 7 days
of each other (identical proximity-match logic to `08b`). Training is patient-level
(one row per patient), using fold assignments from `cv_splits_patients.csv`.
...
Training script: `clinical/train_clinical.py`
Output dir: `/home/jovyan/kgbk271-ibd-volume/training/cd_vs_uc/clinical/results/`
