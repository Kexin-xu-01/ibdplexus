"""
Multimodal CD vs UC — visit-level, at-20-cm, manual-KNN — HISTOSCORE variant.

Same cohort, CV splits, RNA data, clinical features, and classifiers as
train_multimodal.py — the only change is that the imaging modality uses the
11-d prism2 histological scores instead of the 2,560-d prism2_base embeddings.

Arms (each run with RF and MLP)
----------------------------------
  clinical_visit         — 15 clinical features (identical to train_multimodal.py)
  histo_clinical_visit   — histoscores (11) + clinical (15) = 26-d
  rna_clinical_visit     — RNA log2(TPM+1) + clinical (identical to train_multimodal.py)
  trimodal_histo_visit   — histoscores + RNA log2(TPM+1) + clinical

Outputs
-------
  at20cm_visit_manual_knn/results/at20cm_multimodal_histo_fold_metrics.csv
  at20cm_visit_manual_knn/results/at20cm_multimodal_histo_predictions.csv
  at20cm_visit_manual_knn/results/at20cm_multimodal_histo_summary.json
"""
...
def load_img_base_for_matched(img_m):
    """Loaded only to reproduce the BulkFormer-filtered cohort selection.
    prism2_base features themselves are not used in any arm of this script."""
    wsi = pd.read_csv(WSI_META)
    ...
    df = pd.DataFrame(rows)
    print(f'  prism2_base cohort filter: {len(df)} visits have >=1 h5 file')
    return df
...
def main():
    ...
    print('\n=== prism2_base cohort filter (identical to train_multimodal.py) ===')
    base_present = load_img_base_for_matched(img_m)
    has_base = img_m['visit_key'].isin(set(base_present['visit_key']))
    img_m = img_m[has_base.values].reset_index(drop=True)
    rna_m = rna_m[has_base.values].reset_index(drop=True)

    print('\n=== BulkFormer cohort filter ===')
    emb_df = load_bulkformer_emb()
    has_emb = join_bulkformer(rna_m, emb_df)
    ...
    arms = [
        ...
        ('histo_clinical_visit',  img_m, np.hstack([X_clin, X_histo]),               N_CLIN),
        ('rna_clinical_visit',    rna_m, np.hstack([X_clin, X_rna]),                 N_CLIN),
        ('trimodal_histo_visit',  rna_m, np.hstack([X_clin, X_histo, X_rna]),        N_CLIN),
    ]
    ...
    pd.DataFrame(all_fold_results).to_csv(
        os.path.join(OUT_DIR, 'at20cm_multimodal_histo_fold_metrics.csv'), index=False)
    pd.DataFrame(all_preds).to_csv(
        os.path.join(OUT_DIR, 'at20cm_multimodal_histo_predictions.csv'), index=False)
    ...
    with open(os.path.join(OUT_DIR, 'at20cm_multimodal_histo_summary.json'), 'w') as f:
        json.dump(summary_list, f, indent=2)

    print('\n\n=== MULTIMODAL (HISTOSCORE) SUMMARY ===')
    print(f"{'Strategy':<36} {'AUC':<24} {'AP':<24} {'Accuracy'}")
    print('-' * 96)
    for s in summary_list:
        print(f"  {s['strategy']:<34} "
