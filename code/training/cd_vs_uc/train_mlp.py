...
Canonical pipeline: visit-level, at-20-cm, manual-KNN-filtered patches.
Drop-in MLP counterpart to train_rf.py; identical data construction and
cohort (945 visits / 817 patients).

Key differences vs RF:
  - StandardScaler fitted on training fold only (required for MLP)
  - Class imbalance via compute_sample_weight('balanced') in fit()
  - PCA(640) still fitted on training fold only

Arms
-----
  img_base_visit          - prism2 base (2,560-d)
  img_histoscore_visit    - histo scores (11-d)   [concept learning]
  ...
  concat_histoscore_visit - RNA + histo scores    [concept learning]
...
Outputs
-------
  at20cm_visit_manual_knn/results/at20cm_mlp_fold_metrics.csv
  at20cm_visit_manual_knn/results/at20cm_mlp_predictions.csv
  at20cm_visit_manual_knn/results/at20cm_mlp_summary.json
"""
