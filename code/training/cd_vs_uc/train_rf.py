"""
Primary RF classifier — at-20-cm, visit-level, manual-KNN-filtered patches.

Canonical pipeline:
  - Cohort:     945 visits / 817 patients (CD 632, UC 313)
  - Site:       at-20-cm biopsies only
  - Imaging:    prism2_base from tissue_threshold_15_filtered_no_darkspot_manual_knn
  - RNA:        log2(TPM+1) (62,703 genes)
...
-----
  ...
  rna_visit               - RNA-seq log2(TPM+1) (62,703 genes)
  pca640_rna_visit        - PCA(640) of log2(TPM+1)
...
Outputs
-------
  at20cm_visit_manual_knn/results/at20cm_rf_fold_metrics.csv
  at20cm_visit_manual_knn/results/at20cm_rf_predictions.csv
  at20cm_visit_manual_knn/results/at20cm_rf_summary.json
"""
...
def run_arm(name, df, X, pca_dim=None):
    ...
    for fold in range(5):
        ...
        if pca_dim is not None:
            X_rna_part = X[:, :pca_dim]
            X_rest     = X[:, pca_dim:]
            pca = PCA(n_components=pca_dim, random_state=42)
            X_rna_tr = pca.fit_transform(X_rna_part[tr])
            X_rna_va = pca.transform(X_rna_part[va])
            X_tr = np.hstack([X_rna_tr, X_rest[tr]]) if X_rest.shape[1] else X_rna_tr
            X_va = np.hstack([X_rna_va, X_rest[va]]) if X_rest.shape[1] else X_rna_va
        else:
            X_tr = X[tr]
            X_va = X[va]
...
def main():
    ...
    for arm_name, arm_df, arm_X, arm_pca in [
        ('img_base_visit',          img_m,  X_base,                          None),
        ('img_histoscore_visit',    img_m,  X_histo,                         None),
        ('bulkformer_visit',        bf_m,   X_bf,                            None),
        ('rna_visit',               rna_m,  X_rna,                           None),
        ('pca640_rna_visit',        rna_m,  X_rna,                           N_PCA),
        ('concat_raw_visit',        rna_m,  np.hstack([X_rna, X_base]),      None),
        ('concat_histoscore_visit', rna_m,  np.hstack([X_rna, X_histo]),     None),
        ('concat_bf_base_visit',    bf_m,   np.hstack([X_bf,  X_base]),      None),
        ('concat_bf_histo_visit',   bf_m,   np.hstack([X_bf,  X_histo]),     None),
        ('concat_pca640_base',      img_m,  np.hstack([X_rna, X_base]),      N_PCA),
    ]:
        print(f'\n--- {arm_name} ---')
        fr, preds = run_arm(arm_name, arm_df, arm_X, pca_dim=arm_pca)
        all_fold_results += fr; all_preds += preds

    pd.DataFrame(all_fold_results).to_csv(
        os.path.join(OUT_DIR, 'at20cm_rf_fold_metrics.csv'), index=False)
    pd.DataFrame(all_preds).to_csv(
        os.path.join(OUT_DIR, 'at20cm_rf_predictions.csv'), index=False)
    ...
    with open(os.path.join(OUT_DIR, 'at20cm_rf_summary.json'), 'w') as f:
